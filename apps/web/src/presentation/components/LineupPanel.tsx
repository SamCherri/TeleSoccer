...
export function LineupPanel({ lineup, canClaim, isLoading, onClaim }: LineupPanelProps) {
  const homeSlots = lineup.filter((slot) => slot.teamSide === "HOME").sort((a, b) => a.slotNumber - b.slotNumber);
  const awaySlots = lineup.filter((slot) => slot.teamSide === "AWAY").sort((a, b) => a.slotNumber - b.slotNumber);
