...
export const phase1BotActions = {
  ...
  weeklyTraining: '🏃 Treino',
  tryout: '🥅 Peneira',
  confirmTryout: 'Entrar na peneira',
  careerStatus: '🧭 Momento',
  careerHistory: '📜 Carreira',
  walletStatement: '💰 Carteira',
  startMatch: '🏟️ Estádio',
  currentMatch: '🔁 Jogo atual',
  resolveTimeout: '⏭️ Seguir',
  weekAgenda: '🗓️ Agenda',
  lockerRoom: '🚪 Vestiário',
  invitations: '📨 Convites',
  createSession: '📣 Convocar',
  currentSession: '👥 Elenco',
  prepareSession: '🔥 Aquecer',
  joinSessionGuide: '/entrar-sala CODIGO HOME TITULAR',
  trainingPassing: '👟 Passe',
  trainingShooting: '🎯 Finalização',
  trainingDribbling: '🪄 Drible',
  trainingSpeed: '⚡ Velocidade',
  trainingMarking: '🛡️ Marcação',
  trainingReflexes: '🧤 Reflexos',
  cancel: 'Cancelar',
  ...
  matchPass: '👟 Passar',
  matchDribble: '🪄 Driblar',
  matchShoot: '🎯 Finalizar',
  matchControl: '🧲 Dominar',
  matchProtect: '🛡️ Proteger',
  matchTackle: '🦵 Bote',
  matchClear: '💥 Afastar',
  matchSave: '🧤 Defender',
  matchPunch: '✋ Espalmar',
  matchCatch: '🤲 Segurar',
  matchRushOut: '🚀 Sair do gol',
  matchRebound: '↩️ Rebater',
  matchHand: '🫳 Mão',
  matchFoot: '🦶 Pé',
  matchLowLeft: '↙️ Baixo E',
  matchLowRight: '↘️ Baixo D',
  matchHighLeft: '↖️ Alto E',
  matchHighRight: '↗️ Alto D',
  matchDetails: '📖 Detalhes'
} as const;
...
export class Phase1TelegramFacade {
  ...
  async handleCurrentMatch(telegramId: string): Promise<BotReply> {
    ...
    return this.toMatchReply(
      match,
      match.status === MatchStatus.Finished ? '🏁 Último jogo encerrado.' : '🔴 Jogo ao vivo.',
      this.buildWorldActionsFromSnapshot(snapshot)
    );
  }

  async handleCurrentMatchDetails(telegramId: string): Promise<BotReply> {
    const match = await this.getActiveMatchService.execute(telegramId);
    const snapshot = await this.getWorldSnapshot(telegramId);
    return {
      text: this.renderer.renderMatchDetailsCard(match),
      actions: match.status === MatchStatus.Finished || !match.activeTurn
        ? this.buildWorldActionsFromSnapshot(snapshot)
        : [
    ...
    return {
      text: [
        ...
        snapshot.activeMatch
          ? 'Seu jogo está vivo e o estádio é o atalho mais quente.'
          : snapshot.currentSession
            ? 'Seu elenco já está montado e o vestiário virou o próximo passo natural.'
            : snapshot.player.careerStatus === CareerStatus.Professional
              ? 'Seu mundo está livre para rotina, convocação e preparação do próximo confronto.'
              : 'Seu mundo está livre para treino, peneira e crescimento rumo ao profissional.'
      ].join('\n\n'),
